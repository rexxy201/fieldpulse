...
if (method() === 'POST') {
    ...
    if (empty($errors)) {
        if ($editing) {
            ...
            dbRun("UPDATE inv_items SET name=?, category_id=?, description=?, quantity=?, image=?, updated_at=NOW() WHERE id=?",
                [$name, $catId, $desc, $qty, $imageName, $id]);
            ...
            dbRun("INSERT INTO inv_items (unique_code,name,category_id,description,quantity,image) VALUES (?,?,?,?,?,?)",
                [$code, $name, $catId, $desc, $qty, $imageName]);
